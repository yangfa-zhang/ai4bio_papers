...
from core.ONLSTM import OnLSTM
import numpy as np
...
class BLSTM(nn.Module):
    ...
    def forward(self, input):
        ...
        hidden_state = torch.randn(self.num_layers*self.num_directions, batch_size, self.hidden_dim).to(device)  # [num_layers(=1) * num_directions(=2), batch_size, n_hidden]
        cell_state = torch.randn(self.num_layers*self.num_directions, batch_size, self.hidden_dim).to(device)   # [num_layers(=1) * num_directions(=2), batch_size, n_hidden]
